...
3. Gestion des Délais (Cicatrisation et Laboratoire) :
  ...
  JSON SEULEMENT. MAX 40 tâches. Durées: Clinique=1j, Labo=2sem, Cicatrisation=3sem, Implant=4m.

Format: {"taches":[{"id":"T1","nom":"Acte (dents)","phase":1,"duree":{"valeur":1,"unite":"jour"},"dependances":[]}]}

DÉPENDANCES SIMPLIFIÉES (système "after" comme Mermaid):
- Une tâche commence APRÈS qu'une autre se termine
- Format simple: "dependances":["T1"] signifie "commence après T1"
- Plusieurs prérequis: "dependances":["T1","T2"] signifie "commence après T1 ET T2"
- Sans dépendance: "dependances":[] signifie "peut commencer immédiatement"

Règles de séquencement:
- Même secteur dentaire → séquentiel (une tâche après l'autre)
- Secteurs différents → parallèle possible (pas de dépendance)
- Éviter les cycles (A→B→C→A interdit)
- Privilégier la simplicité et la logique médicale`;
}